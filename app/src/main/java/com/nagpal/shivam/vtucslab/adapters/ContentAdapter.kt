...
class ContentAdapter(
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
    return when (viewType) {
      VIEW_TYPE_SSP_SF -> {
        val sspSfBinding =
            LayoutCardSeSspSfBinding.inflate(LayoutInflater.from(context), parent, false)
        SspSfViewHolder(sspSfBinding)
      }
      VIEW_TYPE_MSP_SF -> {
        val mspSfBinding =
            LayoutCardSeMspBinding.inflate(LayoutInflater.from(context), parent, false)
        MspSfViewHolder(mspSfBinding)
      }
      VIEW_TYPE_SSP_MF -> {
        val sspMfBinding =
            LayoutCardSeSspMfBinding.inflate(LayoutInflater.from(context), parent, false)
  ...
  override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
    ...
    when (holder) {
      is SspSfViewHolder -> {
        ...
        val parts =
            labExperiment.labExperimentSubParts[0]
                ...
                .toTypedArray()
        if (parts.size >= 2) {
          holder.binding.programTitle.text = formatProgramName(parts[parts.size - 2])
        }
      }
      is MspSfViewHolder -> {
        holder.binding.serialOrder.text = serialOrder
        val adapter =
            MultipleSubPartAdapter(
                context, labExperiment.labExperimentSubParts, false, itemClickHandler)
      ...
      is SspMfViewHolder -> {
        ...
        val adapter =
            MultipleFileAdapter(
                context, labExperiment.labExperimentSubParts[0].contentFiles, itemClickHandler)
      ...
      is MspMfViewHolder -> {
        ...
        val adapter =
            MultipleSubPartAdapter(
                context, labExperiment.labExperimentSubParts, true, itemClickHandler)
        holder.binding.subPartContainer.adapter = adapter
      }
    }
  }
  ...
  override fun getItemViewType(position: Int): Int {
    ...
    return if (labExperimentSubPartsSize == 1 && contentFileLength == 1) VIEW_TYPE_SSP_SF
    else if (labExperimentSubPartsSize > 1 && contentFileLength == 1) VIEW_TYPE_MSP_SF
    else if (labExperimentSubPartsSize == 1) VIEW_TYPE_SSP_MF
    else if (labExperimentSubPartsSize > 1) VIEW_TYPE_MSP_MF else VIEW_TYPE_INVALID
  }
  ...
  internal inner class SspSfViewHolder(var binding: LayoutCardSeSspSfBinding) :
    ...
    override fun onClick(view: View) {
      itemClickHandler?.let {
        ...
        val contentFile = labExperimentArrayList[position].labExperimentSubParts[0].contentFiles[0]
