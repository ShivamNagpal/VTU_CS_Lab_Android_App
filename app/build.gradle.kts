...
// Install GitHooks
// This snippet is placed in the :app module as the tasks such as build, preBuild doesn't exists in
// the root project.
// However there is a way to place this logic in the rootProject build.gradle.kts by simply moving
// this logic to the rootProject file.
// Then prefix tasks with project(":app") at both th places and wrap the build dependency section
// with afterEvaluate {} as the tasks are generated dynamically.
...
tasks.named("build") { dependsOn(installLocalGitHookTaskName) }
